...
namespace BQ
{
    public class ImportKSVendor
    {
        string conString = DB_Base.DB_STR;
        string path = System.AppDomain.CurrentDomain.BaseDirectory;
        public void ImportVendor(DC_BQ objBQ)
        {
            string sLogFormat = "";
            sLogFormat = " ======== " + DateTime.Now.ToShortDateString().ToString() + " " + DateTime.Now.ToLongTimeString().ToString() + " ======== ";
            CreateErrorLog("CustomLogs/ErrorLog", sLogFormat + " : Vendors");
            try
            {
                DataSet ds = null;
                DeleteTempData();
                ds = getVendorList(objBQ);
                if (ds.Tables[0].Rows[0][0].ToString() != "Vendors not found.")
                {
                    if (ds.Tables.Contains("vendors"))
                    {
                        SaveVendorInTempTable(ds, objBQ);
                    }
                }
            }
            catch (Exception exp)
        ...
        private DataSet getVendorList(DC_BQ objBQ)
        {
            DataSet ds = null;
            //RFI - Domestic: nprijva0sksc2iugl9f5mm4722
            string KSToken = objBQ.KSToken;
            Uri uri = new Uri("https://api.kometsales.com/api/vendor.list?authenticationToken=" + KSToken + "");
            string data = "";
        ...
        private void SaveVendorInTempTable(DataSet _ds, DC_BQ objBQ)
        {
            ...
            if (_ds.Tables[1].Rows.Count > 0)
            {
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(connection))
                {
                    ...
                    bulkCopy.DestinationTableName = "dbo.vendors";
                    foreach (DataColumn column in _ds.Tables["vendors"].Columns)
                    {
