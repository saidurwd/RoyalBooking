...
namespace BQ
{
    public class CreateKSPrebooks
    {
        ...
        public DataSet CreateKSPrebooksByIdVA(DC_BQ objBQ, Prebooks objPB)
        {
            string sLogFormat = "";
            DataSet ds = null;
            try
            {
                ds = CreatePrebooksVA(objBQ, objPB);
            }
            catch (Exception exp)
            {
                sLogFormat = " ======== " + DateTime.Now.ToShortDateString().ToString() + " " + DateTime.Now.ToLongTimeString().ToString() + " ======== ";
        ...
        private DataSet CreatePrebooksVA(DC_BQ objBQ, Prebooks objPB)
        {
            DataSet ds = null;
            string KSToken = objBQ.KSToken;
            ...
            DataSet dsJson = objK.ReadKSPOJSON(objBQ, objPB);
            string json = dsJson.Tables[0].Rows[0][0].ToString();
            json = json.Remove(json.Length - 1); //Remove last one char
            json = json.Substring(1, json.Length - 1); //Remove first one
            //string jsonDetails = dsJson.Tables[1].Rows[0][0].ToString();
            //jsonDetails = jsonDetails.Remove(jsonDetails.Length - 1);
            //jsonDetails = jsonDetails.Substring(1, jsonDetails.Length - 1);
        ...
        public DataSet CreatePrebooksAddProductVA(DC_BQ objBQ, Prebooks objPB, string _prebookId)
        {
            DataSet ds = null;
            string KSToken = objBQ.KSToken;

            var httpWebRequest = (HttpWebRequest)WebRequest.Create("https://api.kometsales.com/api/prebook.vendor.availability.item.add");
            httpWebRequest.ContentType = "application/json";
            httpWebRequest.Method = "POST";

            ReadKSData objK = new BQ.ReadKSData();
            DataSet dsJson = objK.ReadKSPOJSONVA(objBQ, objPB, _prebookId);
